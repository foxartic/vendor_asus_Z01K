...
echo "[ASUS][RTC] Set timezone persist.asus.first_boot = ${first_boot}" > /dev/kmsg
log -p v -t "[ASUS][RTC] Set timezone persist.asus.first_boot = ${first_boot}"
...
if [ "${first_boot}" != 1 ] || [ ! -f /data/property/persist.asus.first_boot ];then
	...
	if [ "$customer" == "VF_IT" ] || [ "$customer" == "WIND" ] || [ "$customer" == "H3G" ];then
		setprop persist.sys.timezone "Europe/Brussels"
		echo "[ASUS][RTC] Set persist.sys.timezone to Europe/Brussels" > /dev/kmsg
		log -p v -t "[ASUS][RTC]" "Set persist.sys.timezone to Europe/Brussels"
	fi
